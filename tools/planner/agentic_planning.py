...
Saves:
  ...
  - llm_response.json  (FULL LLM output)
  - deterministic_response.json      (FULL deterministic output)
...
def main():
    ...
    csv_path = Path(__file__).resolve().parents[2] / f"{args.day_type}day.csv"
    df = pd.read_csv(csv_path)
    n = len(df)
    print(f"[MAIN] CSV size: {n}")
    ...
    for idx in range(start, min(end, n)):
        ...
        llm_path = os.path.join(out_dir, "llm_response.json")
        manual_path = os.path.join(out_dir, "deterministic_response.json")
        non_agentic_path = os.path.join(out_dir, "non_agentic_deterministic_response.json")
